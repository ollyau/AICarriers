...
namespace AICarriers {
    class Log {
        ...
        private string AssemblyLoadDirectory {
            get { return Path.GetDirectoryName(new System.Uri(Assembly.GetExecutingAssembly().CodeBase).LocalPath); }
        }
...
#if DEBUG
            string message = string.Format("{0} Debug {1}", DateTime.Now.ToString("HH:mm:ss.fff"), s);
